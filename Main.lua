...
local function main()
  ...
  paths.dofile('c3dGAP.lua')
  
  model = c3dModel
  
  loadModel = false
  if loadModel ~= true then
    W = torch.load('../sport1m.t7')
    local method = 'xavier'
    model = require('weight-init')(model, method)
    
    --convs = {1,3+1,6+1,7+1,9+1,10+1,12+1,13+1}
  ...
  dualCriterion = nn.ClassNLLCriterion()
  ...
  --Loading Train List  
  trainList = torch.load("trainList.t7")
